...
Page({
  ...
  */
  ...
  /**
   * 获取用户默认地址
   * 如果通过编辑按钮跳过来,这里显示要编辑的;如果通过添加按钮跳过来,显示为空
   */
    ...
    editAddress(value).then(res=>{
      ...
      setTimeout(function () {
        if (that.data.cartId) {
          ...
          that.setData({ cartId: '', pinkId: '', couponId: '' })
          wx.navigateTo({
            url: '/pages/order_confirm/order_confirm?cartId=' + cartId + '&addressId=' + (that.data.id ? that.data.id : res.data.id) + '&pinkId=' + pinkId + '&couponId=' + couponId
          });
        } else {
          wx.navigateBack({ delta: 1 });
        }
