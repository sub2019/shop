...
import { getCouponsOrderPrice } from '../../api/api.js';
...
Page({
  ...
  getConfirm:function(){
    ...
    orderConfirm(this.data.cartId).then(res=>{
      ...
      console.log('错误信息', typeof(err), err)
      // return app.Tips({ title: err }, { tab: 3, url: 1 }); // title的值不能是对象
      return app.Tips({ title: 'error' }, { tab: 3, url: 1 });
    });
  },
  ...
  /**
   * 获取当前金额可用优惠券(getCouponsOrderPrice接口)
   */
  ...
  /**
   * 获取默认收货地址或者获取某条地址信息(getAddressDefault接口, getAddressDetail接口)
   */
